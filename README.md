...
- MicroSD card (signal export & versioning)  
...
No protocol was automatically detected, so all signals were stored and replayed using **RAW capture format** for maximum compatibility.
...
Each device was assigned a unique file with standardized naming conventions for scalability and clarity.

---

### 3. Signal Export & Version Control

All `.ir` files were exported from the Flipper Zero SD card and versioned into this repository to ensure:
...
- RAW IR capture ensures maximum compatibility but increases file size.  
- Some consumer devices require precise angles for optimal signal reception.  
- Consistent naming conventions are critical for long-term scalability and automation.  
